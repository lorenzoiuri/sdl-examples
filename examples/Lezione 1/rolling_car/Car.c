...
void Car_render (SDL_Renderer* renderer, const Car* car) {
   ...
   // render the wheels
   RenderCircle(renderer, car->pos.xPos + 0.25*car->bodyWid, car->pos.yPos + car->bodyHei, car->wheelRadius);
   RenderCircle(renderer, car->pos.xPos + 0.75*car->bodyWid, car->pos.yPos + car->bodyHei, car->wheelRadius);
} 
